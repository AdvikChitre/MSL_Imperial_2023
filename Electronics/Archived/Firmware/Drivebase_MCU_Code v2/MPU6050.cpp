/* Adapted from "MPU6050_light" library for Arduino
 * 
...
uint8_t MPU6050::enableInterrupt() {
  uint8_t data = readData(MPU6050_INT_ENABLE);
  data = data | 0x01; // Set LSB to 1
  uint8_t status = writeData(MPU6050_INT_ENABLE, data);
  return status;
}
